...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'dude':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dude/dudeDialogue'));
			case 'gen-z':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gen-z/generationDialogue'));
		}
			...
			case 'dudeStage':
				{
						defaultCamZoom = 0.9;
						curStage = 'dudeStage';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dudeBGalt'));
						bg.antialiasing = true;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | cry harder engine (KE " + MainMenuState.kadeEngineVer :  ""), 6);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "BOTPLAY (this will not give you a score cry about it)", 20);
						...
						if (daNote.noteType == 3)
							{
								...
								score = -300;
								misses++;
								FlxG.sound.play(Paths.sound('gun_shoot'));
